package com.nfo.core.utils.enums;

public class MongodbEnum {
    public final static String connection = "mongodb+srv://nfoshopdb:nfo.shop123456@cluster0.q2cik.mongodb.net/myFirstDatabase?retryWrites=true&w=majority";
    //    public final static String connection = "mongodb://localhost:27017";
    public final static String database_name = "nfo_db";

    public final static String collection_code = "code";
    public final static String collection_member = "member";
    public final static String collection_auth = "auth";
    public final static String collection_category = "category";
    public final static String collection_product = "product";
}
